...
export function RuleParametersEditor({
  ...
  const isV12Rule =
    ruleId.includes("v12") ||
    ruleName.includes("v12") ||
    ruleName.includes("sinais clássicos");
  ...
  // --------------------------------------------------------------------------
  // 4. REGRA 4: V12 OVER & BACK ALAVANCADO (4 SINAIS CLÁSSICOS UNIFICADOS)
  // --------------------------------------------------------------------------
  if (isV12Rule) {
    const v12: V12RulesConfig = rulesConfig.v12Config || DEFAULT_V12_CONFIG;

    const updateV12 = <K extends keyof V12RulesConfig>(
      section: K,
      patch: Partial<V12RulesConfig[K]>
    ) => {
      const currentSection = v12[section] || DEFAULT_V12_CONFIG[section];
      const updatedV12: V12RulesConfig = {
        ...v12,
        [section]: {
          ...currentSection,
          ...patch,
      ...
      saveConfigPatch({
        v12Config: updatedV12,
        enableV12OverBack: true,
      });
    };

    const resetV12Defaults = () => {
      saveConfigPatch({
    ...
    return (
      <div className="p-4 bg-slate-950/80 border border-cyan-500/40 rounded-2xl space-y-4">
        <div className="flex items-center justify-between flex-wrap gap-2 pb-3 border-b border-slate-800">
          <div className="flex items-center gap-2">
            <Sliders className="w-4 h-4 text-cyan-400" />
            <span className="font-bold text-xs text-white">
              Parâmetros Editáveis V1.2 (4 Sinais Clássicos Unificados)
            </span>
            <span className="px-2 py-0.5 rounded text-[10px] font-extrabold bg-cyan-950 text-cyan-300 border border-cyan-700/60">
              Regra 4
            </span>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={() => saveConfigPatch({ v12Config: { ...v12 } })}
              disabled={isSaving}
              className="flex items-center gap-1 px-3 py-1 rounded-lg bg-cyan-600 hover:bg-cyan-500 text-white font-bold text-xs shadow transition disabled:opacity-50"
            >
              <Save className="w-3.5 h-3.5" />
              <span>{isSaving ? "Salvando..." : "Salvar no Disco"}</span>
            </button>
        ...
        <p className="text-xs text-slate-400 leading-relaxed">
          Avalia simultaneamente os 4 sinais clássicos de alta conversão. Configure os limites analíticos de cada sinal:
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div className="p-3.5 rounded-xl bg-slate-900/90 border border-slate-800 space-y-2">
            <div className="flex items-center justify-between">
              ...
              <button
                type="button"
                onClick={() => updateV12("overPremium", { enabled: v12.overPremium.enabled === false })}
              ...
              <div>
                ...
                <div className="flex items-center gap-1">
                  <input
                    ...
                    onChange={(e) => updateV12("overPremium", { minMinute: parseInt(e.target.value) || 36 })}
                  ...
                  <input
                    ...
                    onChange={(e) => updateV12("overPremium", { maxMinute: parseInt(e.target.value) || 50 })}
                    className="w-full bg-slate-950 border border-slate-700 rounded px-1.5 py-1 text-center font-mono text-white text-xs"
                  />
                </div>
              </div>
              <div>
                <label className="text-slate-400 block text-[10px]">Min CC Total</label>
                <input
                  type="number"
                  value={v12.overPremium.minTotalCc}
                  onChange={(e) => updateV12("overPremium", { minTotalCc: parseInt(e.target.value) || 3 })}
                  className="w-full bg-slate-950 border border-slate-700 rounded px-1.5 py-1 text-center font-mono text-white text-xs"
                />
              </div>
              <div>
                <label className="text-slate-400 block text-[10px]">Max Taxa (min/CC)</label>
                <input
                  type="number"
                  step="0.5"
                  value={v12.overPremium.maxCcRate}
                  onChange={(e) => updateV12("overPremium", { maxCcRate: parseFloat(e.target.value) || 15.0 })}
                  className="w-full bg-slate-950 border border-slate-700 rounded px-1.5 py-1 text-center font-mono text-white text-xs"
                />
              </div>
            </div>
            <p className="text-[10px] text-slate-500 leading-tight">
              Requer CC total ≥ {v12.overPremium.minTotalCc} e taxa de perigo ≤ {v12.overPremium.maxCcRate} min/CC.
            </p>
          </div>
          ...
          <div className="p-3.5 rounded-xl bg-slate-900/90 border border-slate-800 space-y-2">
            <div className="flex items-center justify-between">
              <span className="font-bold text-xs text-amber-300">2. OVER BILATERAL FORTE (36&apos; a 65&apos;)</span>
              <button
                type="button"
                onClick={() => updateV12("overBilateralForte", { enabled: v12.overBilateralForte.enabled === false })}
                className={`text-[10px] px-2 py-0.5 rounded font-bold ${
            ...
            <div className="grid grid-cols-3 gap-2 text-xs">
              <div>
                ...
                <div className="flex items-center gap-1">
                  <input
                    ...
                    onChange={(e) => updateV12("overBilateralForte", { minMinute: parseInt(e.target.value) || 36 })}
                    className="w-full bg-slate-950 border border-slate-700 rounded px-1.5 py-1 text-center font-mono text-white text-xs"
                  />
                  <span className="text-slate-500">-</span>
                  <input
                    ...
                    onChange={(e) => updateV12("overBilateralForte", { maxMinute: parseInt(e.target.value) || 65 })}
                    className="w-full bg-slate-950 border border-slate-700 rounded px-1.5 py-1 text-center font-mono text-white text-xs"
                  />
                </div>
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => updateV12("overBilateralForte", { minTotalCc: parseInt(e.target.value) || 4 })}
                  className="w-full bg-slate-950 border border-slate-700 rounded px-1.5 py-1 text-center font-mono text-white text-xs"
                />
              </div>
              <div>
                <label className="text-slate-400 block text-[10px]">Min CC/Time</label>
                <input
                  type="number"
                  value={v12.overBilateralForte.minTeamCc}
                  onChange={(e) => updateV12("overBilateralForte", { minTeamCc: parseInt(e.target.value) || 2 })}
                  className="w-full bg-slate-950 border border-slate-700 rounded px-1.5 py-1 text-center font-mono text-white text-xs"
                />
              </div>
            </div>
            <p className="text-[10px] text-slate-500 leading-tight">
              Jogo aberto com ambos produzindo chances claras (≥ {v12.overBilateralForte.minTeamCc} CC cada).
            </p>
          </div>
          ...
          <div className="p-3.5 rounded-xl bg-slate-900/90 border border-slate-800 space-y-2">
            <div className="flex items-center justify-between">
              <span className="font-bold text-xs text-rose-300">3. OVER GOL LIMITE (76&apos; a 83&apos;)</span>
              <button
                type="button"
                onClick={() => updateV12("overGolLimite", { enabled: v12.overGolLimite.enabled === false })}
                className={`text-[10px] px-2 py-0.5 rounded font-bold ${
            ...
            <div className="grid grid-cols-3 gap-2 text-xs">
              <div>
                ...
                <div className="flex items-center gap-1">
                  <input
                    ...
                    onChange={(e) => updateV12("overGolLimite", { minMinute: parseInt(e.target.value) || 76 })}
                    className="w-full bg-slate-950 border border-slate-700 rounded px-1.5 py-1 text-center font-mono text-white text-xs"
                  />
                  <span className="text-slate-500">-</span>
                  <input
                    ...
                    onChange={(e) => updateV12("overGolLimite", { maxMinute: parseInt(e.target.value) || 83 })}
                    className="w-full bg-slate-950 border border-slate-700 rounded px-1.5 py-1 text-center font-mono text-white text-xs"
                  />
                </div>
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => updateV12("overGolLimite", { minTotalCc: parseInt(e.target.value) || 7 })}
                  className="w-full bg-slate-950 border border-slate-700 rounded px-1.5 py-1 text-center font-mono text-white text-xs"
                />
              </div>
              <div>
                <label className="text-slate-400 block text-[10px]">Dif. Máx Placar</label>
                <input
                  type="number"
                  value={v12.overGolLimite.minScoreDiff}
                  onChange={(e) => updateV12("overGolLimite", { minScoreDiff: parseInt(e.target.value) || 1 })}
                  className="w-full bg-slate-950 border border-slate-700 rounded px-1.5 py-1 text-center font-mono text-white text-xs"
                />
              </div>
            </div>
            <p className="text-[10px] text-slate-500 leading-tight">
              Final de jogo tenso com CC total ≥ {v12.overGolLimite.minTotalCc} e placar em aberto.
            </p>
          </div>

          {/* Sinal 4: BACK T1 MAIN */}
          <div className="p-3.5 rounded-xl bg-slate-900/90 border border-slate-800 space-y-2">
            <div className="flex items-center justify-between">
              <span className="font-bold text-xs text-emerald-300">4. BACK T1 PRINCIPAL (36&apos; a 50&apos;)</span>
              <button
                type="button"
                onClick={() => updateV12("backT1Main", { enabled: v12.backT1Main.enabled === false })}
                className={`text-[10px] px-2 py-0.5 rounded font-bold ${
              ...
              <div>
                ...
                <div className="flex items-center gap-1">
                  <input
                    ...
                    onChange={(e) => updateV12("backT1Main", { minMinute: parseInt(e.target.value) || 36 })}
                    className="w-full bg-slate-950 border border-slate-700 rounded px-1.5 py-1 text-center font-mono text-white text-xs"
                  />
                  <span className="text-slate-500">-</span>
                  <input
                    ...
                    onChange={(e) => updateV12("backT1Main", { maxMinute: parseInt(e.target.value) || 50 })}
                    className="w-full bg-slate-950 border border-slate-700 rounded px-1.5 py-1 text-center font-mono text-white text-xs"
                  />
                </div>
              </div>
              <div>
                <label className="text-slate-400 block text-[10px]">Min CC Dom.</label>
                <input
                  type="number"
                  value={v12.backT1Main.minDomCc}
                  onChange={(e) => updateV12("backT1Main", { minDomCc: parseInt(e.target.value) || 3 })}
                  className="w-full bg-slate-950 border border-slate-700 rounded px-1.5 py-1 text-center font-mono text-white text-xs"
                />
              </div>
              <div>
                <label className="text-slate-400 block text-[10px]">Min xGOT Dom.</label>
                <input
                  type="number"
                  step="0.05"
                  value={v12.backT1Main.minDomXgot}
                  onChange={(e) => updateV12("backT1Main", { minDomXgot: parseFloat(e.target.value) || 0.50 })}
                  className="w-full bg-slate-950 border border-slate-700 rounded px-1.5 py-1 text-center font-mono text-white text-xs"
                />
              </div>
            </div>
            <p className="text-[10px] text-slate-500 leading-tight">
              Domínio unilateral absoluto do mandante/favorito com adversário inofensivo.
            </p>
          </div>
        </div>

        {statusMessage && (
          <div className="p-2.5 bg-cyan-500/15 border border-cyan-500/40 rounded-xl text-cyan-300 text-xs font-bold flex items-center gap-2">
